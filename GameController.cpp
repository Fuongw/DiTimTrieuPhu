...
void GameController::useSwapQuestion() {
    ...
    // Xác định câu hỏi hiện tại đang là Dễ (index 0-4) hay Khó (5-14)
    if (currentIndex < 5) {
        if (availableEasy.empty()) availableEasy = easyBank;

        if (!availableEasy.empty()) {
            std::random_device rd;
            std::mt19937 g(rd());
            std::shuffle(availableEasy.begin(), availableEasy.end(), g);

            // XÓA CÂU CŨ: Ghi đè câu hỏi mới bốc vào vị trí currentIndex
            current15Qs[currentIndex] = availableEasy.back();
            availableEasy.pop_back();
        }
    } else {
        if (availableHard.empty()) availableHard = hardBank;

        if (!availableHard.empty()) {
            std::random_device rd;
            std::mt19937 g(rd());
            std::shuffle(availableHard.begin(), availableHard.end(), g);

            // XÓA CÂU CŨ: Ghi đè câu hỏi mới bốc vào vị trí currentIndex
            current15Qs[currentIndex] = availableHard.back();
            availableHard.pop_back();
        }
    }

    // Reset lại trạng thái ẩn đáp án để câu hỏi mới hiển thị đầy đủ
    hiddenAnswers.clear();
    hiddenAnswers.resize(4, false);
}
