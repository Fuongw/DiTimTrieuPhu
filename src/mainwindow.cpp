...
{
    ui->setupUi(this);
    // ================= MỚI: ÉP KIỂU TRONG SUỐT CHO 3 NÚT TRỢ GIÚP =================
    // 1. Tắt chế độ vẽ khung 3D mặc định của hệ điều hành
    ui->btn_5050->setFlat(true);
    ui->btn_Swap->setFlat(true);
    ui->btn_CallFriend->setFlat(true);

    // 2. Ép màu nền trong suốt tuyệt đối bằng mã RGBA
    QString lifelineStyle = "QPushButton { background-color: rgba(0, 0, 0, 0); border: none; outline: none; }";

    ui->btn_5050->setStyleSheet(lifelineStyle);
    ui->btn_Swap->setStyleSheet(lifelineStyle);
    ui->btn_CallFriend->setStyleSheet(lifelineStyle);
    // ==============================================================================
    // NẠP FONT PIXEL
    ...
    // Bạn XÓA 2 dòng này đi nhé, vì C++ không cần làm việc này nữa:
...
void MainWindow::handleAnswer(int choiceIndex) {
    ...
    if (choiceChar == currentQ.correctAnswer) {
        if (currentQuestionIndex == 15) {
            ...
            // 1. TÍNH TIỀN VÀ LƯU FILE TRƯỚC
            QString finalMoney = prizeMoney[14]; // Đã thắng câu 15 thì chắc chắn ẵm trọn mốc cuối cùng!

            std::ofstream outfile("data/leaderboard.txt", std::ios_base::app);
            if(outfile.is_open()) {
                outfile << currentPlayerName.toStdString() << "," << finalMoney.toStdString() << "\n";
                outfile.close();
            }

            // 2. RỒI MỚI RESET LẠI TỪ ĐẦU
            currentQuestionIndex = 1;
        ...
        // ================= GHI FILE KHI THUA CUỘC =================
        QString finalMoney = (currentQuestionIndex > 1) ? prizeMoney[currentQuestionIndex - 2] : "0";
        std::ofstream outfile("data/leaderboard.txt", std::ios_base::app);
        if(outfile.is_open()) {
            outfile << currentPlayerName.toStdString() << "," << finalMoney.toStdString() << "\n";
            outfile.close();
        ...
        currentQuestionIndex = 1; // Sau đó hệ thống mới reset game
        controller.startNewGame();
        ui->btn_5050->setVisible(false);
        ui->btn_Swap->setVisible(false);
        ui->btn_CallFriend->setVisible(false);
...
void MainWindow::on_btn_Swap_clicked() {
    ...
    // 1. Gọi controller thực hiện đổi câu hỏi (Truyền thẳng vị trí câu hiện tại vào ngoặc)
    // (Đã xóa dòng setCurrentIndex thừa để tránh lỗi)
    controller.useSwapQuestion(currentQuestionIndex);

    // 2. Làm mờ và khóa nút đổi câu hỏi (Giữ nguyên logic tuyệt vời của bạn)
    ui->btn_Swap->setEnabled(false);
    ui->btn_Swap->setStyleSheet("QPushButton { color: gray; background-color: rgba(100, 100, 100, 0.5); }");

    QMessageBox::information(this, "Đổi câu hỏi", "Đã hủy bỏ câu hỏi cũ và đổi câu mới thành công!");
